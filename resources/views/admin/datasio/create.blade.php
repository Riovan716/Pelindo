...
@section('content')
    <h1>Tambah Data SIO</h1>
    <form method="POST" action="{{ route('admin.datasio.store') }}" enctype="multipart/form-data" style="max-width:500px; margin:30px auto; background:#f9f9f9; border-radius:10px; box-shadow:0 2px 8px #0001; padding:24px;">
        @csrf
        ...
        <input type="date" name="tanggal_mulai" value="{{ old('tanggal_mulai') }}" style="width:100%;margin-bottom:8px;"><br>
        @error('tanggal_mulai') <span style="color:red">{{ $message }}</span><br> @enderror

        <label>Tanggal Berakhir</label><br>
        <input type="date" name="tanggal_berakhir" value="{{ old('tanggal_berakhir') }}" style="width:100%;margin-bottom:8px;"><br>
